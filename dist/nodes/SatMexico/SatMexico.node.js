...
async function obtenerIndicadores(that) {
...
async function obtenerUdi(that, fecha) {
    ...
    try {
        ...
        return {
            indicador: 'UDI',
            valor: (data === null || data === void 0 ? void 0 : data.udi) || 8.25,
...
async function obtenerTipoCambio(that, moneda) {
    try {
        const response = await that.helpers.httpRequest({
            method: 'GET',
...
function convertirUdiPesos(udi, valorUdi) {
    return {
        udi,
        valorUdi,
...
function convertirPesosUdi(pesos, valorUdi) {
    return {
        pesos,
        valorUdi,
...
async function emitirCfdi(that, provider, apiKey, tipoCfdi, datos) {
    if (provider === 'none') {
            ...
            try {
                ...
                // Indicadores
                    ...
                    if (operation === 'udi') {
                        ...
                        const montoUdi = this.getNodeParameter('montoUdi', i);
                        const udi = await obtenerUdi(this);
                        result = convertirUdiPesos(montoUdi, udi.valor);
                    }
                    else if (operation === 'pesos_udi') {
                        const montoPesos = this.getNodeParameter('montoPesos', i);
                        const udi = await obtenerUdi(this);
                        result = convertirPesosUdi(montoPesos, udi.valor);
                    }
                }
                // CFDI
                    ...
                    const datos = {
                        receptor: {
                            rfc: this.getNodeParameter('rfcReceptor', i),
                            razonSocial: this.getNodeParameter('razonSocial', i),
                        },
                        usoCfdi: this.getNodeParameter('usoCfdi', i),
                        formaPago: this.getNodeParameter('formaPago', i),
                        items: this.getNodeParameter('items', i),
                    };
                    const tipoCfdi = operation === 'factura' ? 'I' : 'E';
                    result = await emitirCfdi(this, credentials.provider, credentials.facturapiApiKey, tipoCfdi, datos);
                }
